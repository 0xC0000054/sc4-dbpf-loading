...
//
// The search order used for GetResourceKeyList and ConsolidateDatabaseRecords is different from the
// .DAT implementation in MultiPackedFile due to the behavior of the code it is replacing.
//
// MultiPackedFile uses a first in first out search order (front to back), same as SC4's
// cGZPersistDBSegmentMultiPackedFiles class.
//
// LooseSC4MultiPackedFile uses a last in first out search order (back to front) search order, same as SC4's
// cGZPersistResourceManager class.
class LooseSC4MultiPackedFile final : public cRZBaseUnkown, public cIGZPersistDBSegment, public cIGZPersistDBSegmentMultiPackedFiles
{
public:
